...
const ProjectTasks = ({ projectId, sprint_id }: ProjectTasksProps) => {
  ...
  const fetchTasks = useCallback(async () => {
    ...
    try {
      ...
      let url = `https://collab-eijr.onrender.com/project/view_tasks?project_id=${projectId}`;
      if (sprint_id) {
  ...
  const handleStatusChange = async (taskId: number, newStatus: string) => {
    ...
    try {
      const response = await fetch("https://collab-eijr.onrender.com/project/edit_tasks/update_task", {
        method: "POST",
        credentials: "include",
        headers: {
          "Content-Type": "application/json",
